...
func _process(_delta: float) -> void:
	...
	var alignement: float = get_direction_alignment(car)
	
	if alignement < alignement_error_threshold:
		print("WRONG WAY!")
...
func get_direction_alignment(kart: KartController) -> float:
	...
	var alignement: float = -kart.car_mesh_controller.global_transform.basis.z.dot(pos_xform.basis.z)
